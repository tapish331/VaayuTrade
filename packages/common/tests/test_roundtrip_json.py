...
from vaayutrade_common import (
    ...
    PriceLevel, TickSnapshot, AlertEvent, AlertSeverity, BacktestRun, ConfigBlob
)
...
def test_order_limit_roundtrip():
    obj = Order(client_id="cid-1", symbol="TCS", side=OrderSide.BUY, qty=10,
                type=OrderType.LIMIT, limit_price=100.5)
    assert roundtrip(obj) == obj
...
def test_risklimit_roundtrip():
    obj = RiskLimit(max_daily_loss=0.015, per_trade_risk=0.0035, max_positions=10, max_spread_bps=12)
    assert roundtrip(obj) == obj

def test_pnlminute_roundtrip():
    obj = PnLMinute(ts=datetime.now(timezone.utc), realized=10.0, unrealized=-2.0, fees=0.5, turnover=100000)
    assert roundtrip(obj) == obj

def test_modelartifact_roundtrip():
    obj = ModelArtifact(version="1.0.0", path="s3://models/model.onnx", schema_hash="abcd1234",
                        metrics={"auc": 0.71}, calib={"iso_rmse": 0.04})
    assert roundtrip(obj) == obj
...
def test_ticksnapshot_roundtrip():
    obj = TickSnapshot(
        ts=datetime.now(timezone.utc), symbol="TCS",
        bids=[PriceLevel(price=100.0, qty=50)],
        asks=[PriceLevel(price=100.1, qty=60)],
        last=100.05, volume=12345
    )
    assert roundtrip(obj) == obj

def test_alertevent_roundtrip():
    obj = AlertEvent(ts=datetime.now(timezone.utc), type="squareoff.warn", severity=AlertSeverity.WARN, payload={"t": "15:10"})
    assert roundtrip(obj) == obj

def test_backtestrun_roundtrip():
    obj = BacktestRun(config_ref="cfg:2025-08-01", metrics={"sharpe": 1.2}, trades=[{"symbol":"TCS","r":0.5}], seed=42)
    assert roundtrip(obj) == obj
